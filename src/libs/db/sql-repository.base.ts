...
import { AggregateRoot, Mapper, Paginated, PaginatedQueryParams, RepositoryPort } from '@libs/ddd';
  ...
  implements RepositoryPort<Aggregate>
{
  ...
  async insert(entity: Aggregate | Aggregate[]): Promise<void> {
    ...
    try {
      ...
      if (error instanceof UniqueIntegrityConstraintViolationError) {
        this.logger.debug(`[${RequestContextService.getRequestId()}] ${error.stack}`);
   ...
   */
  protected generateInsertQuery(models: DbModel[]): QuerySqlToken<ZodTypeAny> {
    ...
    const query = sql.unsafe`
      INSERT INTO ${sql.identifier([this.tableName])} (${sql.join(propertyNames, sql.fragment`, `)})
