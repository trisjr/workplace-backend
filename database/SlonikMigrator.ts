...
import { DatabasePool, DatabasePoolConnection, DatabaseTransactionConnection, IdentifierSqlToken, sql } from 'slonik';
import {
...
export class SlonikMigrator extends Umzug<SlonikMigratorContext> {
  constructor(
  ...
  getCli(options?: CommandLineParserOptions): UmzugCLI {
    const cli = super.getCli({
      toolDescription: `@slonik/migrator - PostgreSQL migration tool`,
      ...options,
    });
  ...
  protected migrationTableNameIdentifier(): IdentifierSqlToken {
  ...
  protected resolver(params: MigrationParams<SlonikMigratorContext>): RunnableMigration<SlonikMigratorContext> {
  ...
  protected async getOrCreateMigrationsTable(context: SlonikMigratorContext): Promise<void> {
  ...
  async runCommand<T>(command: string, cb: (params: { context: SlonikMigratorContext }) => Promise<T>): Promise<T> {
    let run = cb;
  ...
  async repair(options?: RepairOptions): Promise<void> {
    ...
    await this.runCommand('repair', async ({ context }) => {
      ...
      if (migrationsThatNeedRepair.length === 0) {
        this.slonikMigratorOptions.logger?.info({
  ...
  protected async executedNames({ context }: { context: SlonikMigratorContext }): Promise<string[]> {
    const infos = await this.executedInfos(context);
  ...
  private async executedInfos(context: SlonikMigratorContext): Promise<MigrationInfo[]> {
    await this.getOrCreateMigrationsTable(context);
    const migrations = await context.parent.any(
      sql.unsafe`select name, hash from ${this.migrationTableNameIdentifier()}`,
    );
  ...
  protected async logMigration({ name, context }: { name: string; context: SlonikMigratorContext }): Promise<void> {
  ...
  protected async unlogMigration({ name, context }: { name: string; context: SlonikMigratorContext }): Promise<void> {
    await context.connection.query(sql.unsafe`
      delete from ${this.migrationTableNameIdentifier()}
      where name = ${name}
    `);
  }
  protected async repairMigration({
    name,
    hash,
    context,
  }: {
...
export type Migration = (
  params: MigrationParams<SlonikMigratorContext> & {
...
/**
 * Narrowing of @see UmzugOptions where the migrations input type specifically, uses `glob`
 */
export type SlonikUmzugOptions = UmzugOptions<SlonikMigratorContext> & {
  migrations: GlobInputMigrations<SlonikMigratorContext>;
};
...
const MESSAGE_FORMATS = createMessageFormats({
  ...
  migrated: (msg) => {
    const { name, durationSeconds, ...rest } = msg;
